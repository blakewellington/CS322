...
class IR0Gen2 {
  ...
  static List<IR0.Inst> gen(Ast0.If n) throws Exception {
    List<IR0.Inst> code = new ArrayList<IR0.Inst>();
    Ast0.BOP[] condExps = {Ast0.BOP.EQ, Ast0.BOP.NE, Ast0.BOP.LT, Ast0.BOP.GT, Ast0.BOP.LE, Ast0.BOP.GE};

    IR0.Label L1 = new IR0.Label();
    if (n.cond instanceof Ast0.Binop) {
      if (isRelOp(((Ast0.Binop) n.cond).op)) {
         System.out.println("RelOp!");
         code.add(new IR0.CJump(...)
//  =============  LEFT OFF HERE (ABOVE)  ==================
      }
    }
    else {
  ...
  static List<IR0.Inst> gen(Ast0.While n) throws Exception {
    List<IR0.Inst> code = new ArrayList<IR0.Inst>();
    List<Ast0.BOP> condExps = new ArrayList<Ast0.BOP>();
    Collections.addAll(condExps, Ast0.BOP.EQ, Ast0.BOP.NE, Ast0.BOP.LT, Ast0.BOP.GT, Ast0.BOP.LE, Ast0.BOP.GE);

    if (condExps.contains(n.cond)) {
...
//        if l.src evals to FALSE, jump to L
          code.add(new IR0.CJump(IR0.RelOP.EQ, l.src, IR0.FALSE, L));
//        if r.src evals to FALSE, jump to L
          code.add(new IR0.CJump(IR0.RelOP.EQ, r.src, IR0.FALSE, L));
//        Set t = TRUE
...
//        Set t = TRUE
          code.add(new IR0.Move(t, IR0.TRUE));
//        if l.src evals to TRUE, jump to L
          code.add(new IR0.CJump(IR0.RelOP.EQ, l.src, IR0.TRUE, L));
//        if r.src evals to TRUE, jump to L
          code.add(new IR0.CJump(IR0.RelOP.EQ, r.src, IR0.TRUE, L));
//        Set t = FALSE
          code.add(new IR0.Move(t, IR0.FALSE));
          code.add(new IR0.LabelDec(L));
          break;
